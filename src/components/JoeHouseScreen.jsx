...
function getTrustInfo(trust) {
  ...
  for (const item of TRUST_LEVELS) if (trust >= item.required) current = item;
  ...
  return { current, next, progress: Math.max(0, Math.min(100, (value / range) * 100)), text: `${value}/${range}` };
}
...
export default function JoeHouseScreen({
  ...
  const finishQuest = ({ force = false } = {}) => {
    ...
    const nextInfo = getTrustInfo(nextTrust);
